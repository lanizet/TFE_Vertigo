...
def weights_relevance(weights, feat):
    ...
    for i in range(len(weights)):
        k = len(weights[0])
        weigths_ordered = np.argsort(weights[i])
        for j in weigths_ordered:
            dtf[feat[j]] += k
            k = k-1
    dtf = dtf.sort_values(by=[0], axis=1)
...
for it in ['Three']:
    regions_kept = pd.read_pickle(path+"feature_selection2/regions_selected_corr_struct_%s.p"%it)
    features = features[regions_kept[0]].abs()
    features.to_csv(path+"wrapped_features_selection/%s/struct/rft/features_kept_%s.csv"%(it,it))
    ...
    features.to_csv("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection/%s/struct/rft/features_kept_redundant_%s.csv"%(it,it))
    ...
    count10 = 0
    for i in range(50):
        params_best_rft = find_best_params_rft(features, target, sklearn.ensemble.RandomForestClassifier(), param_rft)
    ...
    params = {"SFS": 35}
    r_list = Parallel(n_jobs=15)(delayed(machine_learning_func)(features, target, num_feats_keep, model_chosen = "rft", params = tab[max]) for i in range(params.get("SFS")))
    
    r_dtf = r_list[0]
    for il in range(1,len(r_list)):
        r_dtf = pd.concat([r_dtf, r_list[il]], ignore_index=True)
    print(r_dtf)
    r_dtf.to_pickle("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection/%s/struct/rft/r_dtf_%s.p"%(it, it))
    ...
    for m in num_feats_keep:
        ...
        inc_m += 1   
    pd.DataFrame(stability).to_csv("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection/%s/struct/rft/stability_%s.csv"%(it, it))
    ...
    df.to_csv("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection/%s/struct/rft/accuracy_%s.csv"%(it, it))

    weights_dtf.to_csv("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection/%s/struct/rft/weigts_%s.csv"%(it, it))
    weights_dtf.to_pickle("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection/%s/struct/rft/weigts_%s.p"%(it, it))

    weights_ordered = get_weights_ordered(weights_dtf, params)
    final_importance_001 = importance_weights(weights_ordered, num_feats_keep[0], params)
    final_importance_002 = importance_weights(weights_ordered, num_feats_keep[1], params)
    final_importance_003 = importance_weights(weights_ordered, num_feats_keep[2], params)
    final_importance_004 = importance_weights(weights_ordered, num_feats_keep[3], params)
    
    final_importance_001.to_csv("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection/%s/struct/rft/final_importance_005_%s.csv"%(it, it))
    final_importance_001.to_pickle("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection/%s/struct/rft/final_importance_005_%s.p"%(it, it))
    final_importance_002.to_csv("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection/%s/struct/rft/final_importance_01_%s.csv"%(it, it))
    final_importance_002.to_pickle("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection/%s/struct/rft/final_importance_01_%s.p"%(it, it))
    final_importance_003.to_csv("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection/%s/struct/rft/final_importance_025_%s.csv"%(it, it))
    final_importance_003.to_pickle("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection/%s/struct/rft/final_importance_025_%s.p"%(it, it))
    final_importance_004.to_csv("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection/%s/struct/rft/final_importance_05_%s.csv"%(it, it))
    final_importance_004.to_pickle("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection/%s/struct/rft/final_importance_05_%s.p"%(it, it))
    ...
    mean_per_num_feat_kept.to_csv("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection/%s/struct/rft/mean_accuracy_%s.csv"%(it, it))
