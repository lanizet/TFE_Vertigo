...
for it in ['Three']:
    regions_kept = pd.read_pickle("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/feature_selection2/regions_selected_MI_func_%s.p"%it)#pd.read_csv("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/feature_selection2/regions_selected_corr_1.csv")
    features = features[regions_kept[0]].abs()
    features.to_csv("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection_MI/%s/func/svc/features_kept_%s.csv"%(it,it))
    ...
    num_feats_keep = (np.array([0.18, 0.35, 0.45, 0.55])*features.shape[1]).astype(int)
    ...
    for i in range(50):
        ...
        #print(params_best_rft.get("n_estimators"))
    ...
    param = {'SFS':35}
    r_list = Parallel(n_jobs=15)(delayed(machine_learning_func)(features, target, num_feats_keep, model_chosen = "svc", params = tab[max]) for i in range(param.get("SFS")))
    
    r_dtf = r_list[0]
    for il in range(1,len(r_list)):
        r_dtf = pd.concat([r_dtf, r_list[il]], ignore_index=True)
    print(r_dtf)
    r_dtf.to_pickle("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection_MI/%s/func/svc/r_dtf2_%s.p"%(it, it))
    ...
    for m in num_feats_keep:
        ...
        inc_m += 1   
    pd.DataFrame(stability).to_csv("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection_MI/%s/func/svc/stability2_%s.csv"%(it, it))
    ...
    df.to_csv("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection_MI/%s/func/svc/accuracy2_%s.csv"%(it, it))

    weights_dtf_H_VCNC .to_csv("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection_MI/%s/func/svc/weigts_H_VCNC2 _%s.csv"%(it, it))
    weights_dtf_H_VCNC .to_pickle("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection_MI/%s/func/svc/weigts_H_VCNC2 _%s.p"%(it, it))
    weights_dtf_HVC_VNC.to_csv("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection_MI/%s/func/svc/weigts_HVC_VNC2_%s.csv"%(it, it))
    weights_dtf_HVC_VNC.to_pickle("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection_MI/%s/func/svc/weigts_HVC_VNC2_%s.p"%(it, it))

    #Control participants VS vertigo participants who have or not compensated
    weights_ordered_H_VCNC = get_weights_ordered(weights_dtf_H_VCNC, "Weights_H_VCNC", param)
    weights_ordered_H_VCNC.to_csv("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection_MI/%s/func/svc/weights_ordered2_%s.csv"%(it, it))
    ...
    final_importance_045_H_VCNC.to_csv("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection_MI/%s/func/svc/final_importance_018_H_VCNC_%s.csv"%(it, it))
    final_importance_045_H_VCNC.to_pickle("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection_MI/%s/func/svc/final_importance_018_H_VCNC_%s.p"%(it, it))
    final_importance_055_H_VCNC.to_csv("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection_MI/%s/func/svc/final_importance_035_H_VCNC_%s.csv"%(it, it))
    final_importance_055_H_VCNC.to_pickle("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection_MI/%s/func/svc/final_importance_035_H_VCNC_%s.p"%(it, it))
    final_importance_065_H_VCNC.to_csv("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection_MI/%s/func/svc/final_importance_045_H_VCNC_%s.csv"%(it, it))
    final_importance_065_H_VCNC.to_pickle("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection_MI/%s/func/svc/final_importance_045_H_VCNC_%s.p"%(it, it))
    final_importance_075_H_VCNC.to_csv("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection_MI/%s/func/svc/final_importance_055_H_VCNC_%s.csv"%(it, it))
    final_importance_075_H_VCNC.to_pickle("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection_MI/%s/func/svc/final_importance_055_H_VCNC_%s.p"%(it, it))
    
    #Control participants and compensated vertigo participants VS vertigo participants
    weights_ordered_HVC_VNC = get_weights_ordered(weights_dtf_HVC_VNC, "Weights_HVC_VNC", param)
    weights_ordered_HVC_VNC.to_pickle("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection_MI/%s/func/svc/weights_ordered_HVC_VNC2_%s.p"%(it, it))
    ...
    final_importance_045_HVC_VNC.to_csv("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection_MI/%s/func/svc/final_importance_018_HVC_VNC_%s.csv"%(it, it))
    final_importance_045_HVC_VNC.to_pickle("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection_MI/%s/func/svc/final_importance_018_HVC_VNC_%s.p"%(it, it))
    final_importance_055_HVC_VNC.to_csv("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection_MI/%s/func/svc/final_importance_035_HVC_VNC_%s.csv"%(it, it))
    final_importance_055_HVC_VNC.to_pickle("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection_MI/%s/func/svc/final_importance_035_HVC_VNC_%s.p"%(it, it))
    final_importance_065_HVC_VNC.to_csv("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection_MI/%s/func/svc/final_importance_045_HVC_VNC_%s.csv"%(it, it))
    final_importance_065_HVC_VNC.to_pickle("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection_MI/%s/func/svc/final_importance_045_HVC_VNC_%s.p"%(it, it))
    final_importance_075_HVC_VNC.to_csv("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection_MI/%s/func/svc/final_importance_055_HVC_VNC_%s.csv"%(it, it))
    final_importance_075_HVC_VNC.to_pickle("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection_MI/%s/func/svc/final_importance_055_HVC_VNC_%s.p"%(it, it))
    ...
    mean_per_num_feat_kept.to_csv("/CECI/proj/pilab/PermeableAccess/vertige_LEWuQhzYs9/wrapped_features_selection_MI/%s/func/svc/mean_accuracy2_%s.csv"%(it, it))
